...
public class SoftKeyboard {
  ...
  public void setFlags(boolean cacheFlag, boolean stickyFlag, boolean isQwerty,
      boolean isQwertyUpperCase) {
  ...
  public void setSkbCoreSize(int skbCoreWidth, int skbCoreHeight) {
    if (null == mKeyRows || (skbCoreWidth == mSkbCoreWidth && skbCoreHeight == mSkbCoreHeight)) {
      return;
    }
  ...
  public SoftKey mapToKey(int x, int y) {
    ...
    for (int row = 0; row < rowNum; row++) {
      KeyRow keyRow = mKeyRows.get(row);
      if (KeyRow.ALWAYS_SHOW_ROW_ID != keyRow.mRowId && keyRow.mRowId != mEnabledRowId) continue;
  ...
  @Override public String toString() {
    ...
    for (int row = 0; row < rowNum; row++) {
      ...
      int keyNum = softKeys.size();
      for (int i = 0; i < softKeys.size(); i++) {
        str += "-key " + String.valueOf(i) + ":" + softKeys.get(i).toString();
      }
    }
    return str + endStr;
  }
