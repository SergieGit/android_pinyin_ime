...
public class PinyinIME extends InputMethodService {
  ...
  private boolean processKey(KeyEvent event, boolean realAction) {
    ...
    if (mInputModeSwitcher.isEnglishWithSkb()) {
      ...
      if (mImeState == ImeState.STATE_IDLE || mImeState == ImeState.STATE_APP_COMPLETION) {
  ...
  private boolean processStatePredict(int keyChar, int keyCode, KeyEvent event,
      boolean realAction) {
    if (!realAction) return true;
  ...
  private boolean processStateEditComposing(int keyChar, int keyCode, KeyEvent event,
      boolean realAction) {
    if (!realAction) return true;
      ...
      if ('\'' != event.getUnicodeChar(event.getMetaState())) {
        ...
        if (0 != fullwidth_char) {
          ...
          if (ComposingView.ComposingStatus.SHOW_STRING_LOWERCASE == cmpsvStatus) {
            retStr = mDecInfo.getOriginalSplStr().toString();
          } else {
  ...
  private void inputCommaPeriod(String preEdit, int keyChar, boolean dismissCandWindow,
      ImeState nextState) {
    if (keyChar == ',') {
      preEdit += '\uff0c';
    } else if (keyChar == '.') {
  ...
  public void responseSoftKeyEvent(SoftKey sKey) {
    ...
    if (sKey.isUserDefKey()) {
      ...
      if (sKey.isKeyCodeKey()) {
        ...
        KeyEvent eUp = new KeyEvent(0, 0, KeyEvent.ACTION_UP, keyCode, 0, 0, 0, 0,
            KeyEvent.FLAG_SOFT_KEYBOARD);
        ...
        if (mInputModeSwitcher.isChineseTextWithSkb() && (ImeState.STATE_INPUT == mImeState
  ...
  @Override public void onStartInputView(EditorInfo editorInfo, boolean restarting) {
    if (mEnvironment.needDebug()) {
  ...
  private void onChoiceTouched(int activeCandNo) {
    if (mImeState == ImeState.STATE_COMPOSING) {
      changeToStateInput(true);
    } else if (mImeState == ImeState.STATE_INPUT || mImeState == ImeState.STATE_PREDICT) {
  ...
  private class PopupTimer extends Handler implements Runnable {
    ...
    void postShowFloatingWindow() {
      mFloatingContainer.measure(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
    ...
    public void run() {
      ...
      if (!mFloatingWindow.isShowing()) {
        ...
        mFloatingWindow.update(mParentLocation[0], mParentLocation[1] - mFloatingWindow.getHeight(),
            mFloatingWindow.getWidth(), mFloatingWindow.getHeight());
      }
    }
  }
  ...
  public class OnGestureListener extends GestureDetector.SimpleOnGestureListener {
    ...
    private boolean mResponseGestures;
    ...
    public OnGestureListener(boolean responseGestures) {
      mResponseGestures = responseGestures;
    }
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
      ...
      if (vXTotal > VELOCITY_THRESHOLD_X2 && absVYTotal < VELOCITY_THRESHOLD_Y2) {
        if (mResponseGestures) onDirectionGesture(Gravity.RIGHT);
        mGestureRecognized = true;
      } else if (vXTotal < -VELOCITY_THRESHOLD_X2 && absVYTotal < VELOCITY_THRESHOLD_Y2) {
        if (mResponseGestures) onDirectionGesture(Gravity.LEFT);
        mGestureRecognized = true;
      } else if (vYTotal > VELOCITY_THRESHOLD_Y2 && absVXTotal < VELOCITY_THRESHOLD_X2) {
        if (mResponseGestures) onDirectionGesture(Gravity.BOTTOM);
        mGestureRecognized = true;
      } else if (vYTotal < -VELOCITY_THRESHOLD_Y2 && absVXTotal < VELOCITY_THRESHOLD_X2) {
        if (mResponseGestures) onDirectionGesture(Gravity.TOP);
        mGestureRecognized = true;
      }
  ...
  public class DecodingInfo {
     ...
     * to indicate that whether the position is counted in spelling id or
     * character.
     */
        ...
        try {
          if (candId < 0) {
            if (length() == 0) {
              ...
              if (mPyBuf == null) mPyBuf = new byte[PY_STRING_MAX];
    ...
    private void updateDecInfoForSearch(int totalChoicesNum) {
      ...
      try {
        ...
        if (mCursorPos > mSurface.length()) mCursorPos = mSurface.length();
        mComposingStr =
            mFullSent.substring(0, mFixedLen) + mSurface.substring(mSplStart[mFixedLen + 1]);
